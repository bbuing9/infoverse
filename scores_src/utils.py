...
def merge_multiple_models(args, loader, backbone, list_epoch, list_seed):
    ...
    for i in range(n_epochs):
        for j in range(n_seeds):
            # Load ith model
            pre_ckpt_i = f"./logs/{args.dataset}_R{args.data_ratio}_{args.backbone}_{args.name}_GA1_N1.0_S{list_seed[j]}/"
            if 'large' in args.backbone:
                pre_ckpt_i += f"{args.dataset}_roberta-large_{args.name}_epoch{list_epoch[i]}.model"
            else:
                pre_ckpt_i += f"{args.dataset}_roberta-base_{args.name}_epoch{list_epoch[i]}.model"

            model_i = Classifier(args.backbone, backbone, args.n_classes, args.train_type).cuda()
